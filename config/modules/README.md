...
The executable v1.6 development manifest catalog is declared by `subsystems/operations/engines/catalog.py` and remains available through `modules.catalog`. The v1.5 and older manifest constants remain unchanged. This directory is reserved for validated serialized manifests if package discovery is introduced later.
