...
v1.7.1은 등록된 각 구성요소 Database에도 SQLite online backup, checksum, Integrity 검증, 허용된 backup root 검증, restore 전 safety backup, restore 후 Integrity 검증과 실패 시 safety rollback을 동일하게 적용한다. Database Management는 이 제어 기능만 호출하며 업무 Table을 직접 수정하지 않는다.
